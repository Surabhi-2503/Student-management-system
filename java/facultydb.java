 ...
 * @author Surabhi
 */
public class facultydb extends HttpServlet {
        ...
        try  {
            ...
            HttpSession s=request.getSession();
              // String  ACCID=(String)s.getAttribute("AID");
               ...
               String  SUBHAN=(String)s.getAttribute("ASUBHD");
               ...
               if(!AUSID.equals(null)&&!AUSID.equals(""))
                {
                    ...
                    stmt3.executeQuery("insert into accountant(name,passwd,address,phone,email,subject_handling) values('"+ANAME+"','"+APASWD+"','"+AADDRESS+"',"+APHONE+",'"+AUSID+"@faculties.com','"+SUBHAN+"')");
                    

                    con3.close();
                
                    out.println("<script>alert(\" entry successfull\")</script>");
                    out.println("<meta http-equiv='refresh' content='0;URL=addflty'>");
            out.println("</body>");
            out.println("</html>");
        }else{
                response.sendRedirect("addflty");
